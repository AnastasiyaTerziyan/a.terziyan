...
public class RPNForm {
		...
		while (tokenizer.hasMoreTokens()) {
			...
			if (oper.isOperator(token)) {
				while ((!stackOper.isEmpty())
						...
						&& (oper.getAccessLevel(token) <= oper
								.getAccessLevel(stackOper.peek()))) {
	...
	private void solveRPN() {
		...
		while (!RPNForm.isEmpty()) {
			...
			if (oper.isInner(token)) {
				try (BufferedReader reader = new BufferedReader(
						new InputStreamReader(new FileInputStream("inner.txt")))) {
					int i = 0;
					String line = "";
			...
			if ((count == 2) && !stackOper.isEmpty()) {
				count = 0;
				String rezult = calculator.calculateExp(stackNum.pop(),
						stackNum.pop(), stackOper.pop());
				stackNum.add(rezult);
			}
		...
		rezult = stackNum.pop();
		try (PrintWriter writer = new PrintWriter(new FileWriter("inner.txt",
				true))) {
			writer.write(rezult);
			writer.println();
		} catch (IOException e) {
